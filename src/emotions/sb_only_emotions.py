...
if not hasattr(torchaudio, "list_audio_backends"):
    def _list_audio_backends():
        # report currently active backend, fallback to sox_io
        backend = None
...
PAD_S = 0.3          # avoid cross-speaker leakage
MIN_SEG_S = 0.4      # skip micro-utterances for stability
...
class SBCategorical:
    ...
    def __init__(self, model_id: str = SB_MODEL_ID, device: str = "cpu"):
    ...
    def load(self):
        ...
        print(f"Loading model from {self.model_id} on {self.device}...")
...
def process(
    ...
    sb = SBCategorical(sb_model_id, device=device).load()
    ...
    n_skipped = sum(
        ...
        and (float(s.get("end", 0)) - float(s.get("start", 0))) < min_seg_s
    )
    ...
    for s in tqdm(segments, desc="Analyzing emotions", unit="seg"):
        ...
        # Initialize categorical emotion fields
        emo_update = {"category_categorical": None, "category_confidence": None}
        ...
        if dur >= min_seg_s:
            ...
            emo_update["category_categorical"] = label
            emo_update["category_confidence"] = conf
            if label is not None:
                processed += 1

        # Merge with any existing emotion dict instead of overwriting
        existing_emo = s.get("emotion", {}) or {}
        existing_emo.update(emo_update)
        s["emotion"] = existing_emo
    ...
    emotions = [
        s.get("emotion", {}).get("category_categorical")
        for s in segments
        if s.get("type") == "speech"
    ]
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="Add SpeechBrain categorical emotions to segments JSON."
    )
    p.add_argument(
        "--audio",
        required=True,
        help="Path to mono WAV/FLAC audio (16 kHz recommended)",
    )
    p.add_argument(
        "--asr_json",
        required=True,
        help="Path to ASR/diarized segments JSON (list of dicts)",
    )
    p.add_argument(
        "--out_json",
        required=True,
    ...
    p.add_argument(
        "--sb_model",
        default=SB_MODEL_ID,
        help="SpeechBrain model id or local folder",
    )
    p.add_argument(
        "--pad",
        type=float,
        default=PAD_S,
        help="Padding around segment in seconds (default 0.0)",
    )
    p.add_argument(
        "--min_seg_s",
        type=float,
        default=MIN_SEG_S,
        help="Min duration (s) to run model (default 0.6)",
    )
    p.add_argument(
        "--device",
        default="cpu",
        help='Torch device string, for example "cpu", "cuda", "cuda:0"',
    )
